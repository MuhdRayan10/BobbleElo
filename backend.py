...
try:
    ...
    client = commands.Bot(
        ...
        application_id="1135592064852181062",
    )  # Setting prefix

    _embed_template = discord.Embed(title="Title!", color=embed_color, url=embed_url)
    ...
    def calculate_team_rating(team1, team2, result):
        ...
        return [round(change1 + p, 2) for p in team1], [
            round(change2 + p, 2) for p in team2
        ]
    ...
    def update_ratings(users, ratings):
        for x in range(len(users)):
            c.execute(
                "UPDATE elo SET rating=:r WHERE userid=:u",
                {"r": ratings[x], "u": users[x]},
            )
        db.commit()
    ...
    class Game(View):
        ...
        async def winner(self, winner):
            ...
            new_elos = calculate_team_rating(
                team1_elo.values(), team2_elo.values(), winner if winner == 1 else 0
            )
        ...
        async def update_embed(self, w, team1, team2):
            ...
            embed.add_field(
                name="Team A",
                value=f'\n{"⬆️" if w == 1 else "⬇️"}'.join(
                    [f"<@{t1[x]}> (`{team1[x]}`)" for x in range(len(t1))]
                ),
            )
            embed.add_field(
                name="Team B",
    ...
    class InitializeGame(View):
        ...
        async def start_game(self, interaction, _):
            if interaction.user.id != self.creator:
                await interaction.response.send_message(
                    "Shh, only the person who created this match can start the game!",
                    ephemeral=True,
                )
                return
            ...
            if not self.team1 or not self.team2:
                await interaction.response.send_message(
                    "You need at least 1 players in each team to start the game!",
                    ephemeral=True,
                )
                return
        ...
        async def cancel_game(self, interaction, _):
            if interaction.user.id != self.creator:
                await interaction.response.send_message(
                    "Shh, only the person who created this match can cancel the game!",
                    ephemeral=True,
                )
                return
